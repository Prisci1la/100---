...
echo "===== Chapter 14 g24 finish run started: $(date) ====="
python - <<'PY'
import torch
...
if [ -f outputs/knock94_beam_bleu.json ]; then
  ...
  python knock94.py --beam-sizes 1,2,5,10 --max-examples 200 \
    2>&1 | tee ../logs/ch14_knock94_limited_g24.log
fi

python - <<'PY' 2>&1 | tee ../logs/ch14_knock95_eval_limited_g24.log
...
python -m torch.distributed.launch --nproc_per_node=8 --use_env knock96.py \
  ...
  2>&1 | tee ../logs/ch14_knock96_limited_g24.log
...
python -m torch.distributed.launch --nproc_per_node=8 --use_env knock97.py \
  ...
  2>&1 | tee ../logs/ch14_knock97_limited_g24.log
...
python knock98.py \
  ...
  --batch-size 32 \
  --max-lines 10000 \
  2>&1 | tee ../logs/ch14_knock98_limited_g24.log

python - <<'PY' 2>&1 | tee ../logs/ch14_knock99_smoke_g24.log
...
echo "===== Chapter 14 g24 finish run finished: $(date) ====="
