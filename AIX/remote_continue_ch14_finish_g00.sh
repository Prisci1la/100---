...
echo "===== Chapter 14 g00 finish run started: $(date) ====="
echo "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES}"

if [ ! -f outputs/knock94_beam_bleu.json ]; then
  python knock94.py --beam-sizes 1,2,5,10 --max-examples 200 \
    2>&1 | tee ../logs/ch14_knock94_limited_g00.log
else
  echo "skip knock94: outputs/knock94_beam_bleu.json exists"
fi

python - <<'PY' 2>&1 | tee ../logs/ch14_knock95_eval_limited_g00.log
...
python -m torch.distributed.launch --nproc_per_node=2 --use_env knock96.py \
  --epochs 1 \
  --batch-size 32 \
  ...
  2>&1 | tee ../logs/ch14_knock96_limited_g00.log

python -m torch.distributed.launch --nproc_per_node=2 --use_env knock97.py \
  --batch-sizes 32,64 \
  --lrs 1e-4,1e-5 \
  ...
  2>&1 | tee ../logs/ch14_knock97_limited_g00.log
...
python knock98.py \
  ...
  --batch-size 32 \
  --max-lines 10000 \
  2>&1 | tee ../logs/ch14_knock98_limited_g00.log

python - <<'PY' 2>&1 | tee ../logs/ch14_knock99_smoke_g00.log
...
echo "===== Chapter 14 g00 finish run finished: $(date) ====="
