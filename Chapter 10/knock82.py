'''
knock82.py: マスクのtop-k予測 / マスクのtop-k予測

"The movie was full of [MASK]."の[MASK]に入るtoken上位10個と確率を求める。
/ "The movie was full of [MASK]."の[MASK]に入るtoken上位10個と確率を求める。
'''
...
def main():  # 定义主函数 / メイン関数を定義する
    parser = argparse.ArgumentParser(description="knock82: predict top-k masked tokens")  # 创建参数解析器 / 引数パーサーを作る
    ...
    parser.add_argument("--top-k", type=int, default=10, help="number of predictions")  # 预测数量 / 予測数
    ...
    predictions = predict_mask_tokens(tokenizer, model, args.sentence, top_k=args.top_k, device=device)  # 预测top-k token / top-k tokenを予測する

    print("=" * 50)  # 输出分隔线 / 区切り線を出力する
    print("Knock 82: Top-k Mask Prediction")  # 输出标题 / タイトルを出力する
    print("=" * 50)  # 输出分隔线 / 区切り線を出力する
    print(f"device: {device}")  # 输出设备 / デバイスを出力する
    print(f"sentence: {args.sentence}")  # 输出输入句子 / 入力文を出力する
    for rank, (token, probability) in enumerate(predictions, start=1):  # 遍历预测结果 / 予測結果を走査する
        print(f"{rank:02d}\t{token}\t{probability:.6f}")  # 输出排名、token、概率 / 順位、token、確率を出力する
...
実行結果 / 运行结果:
==================================================
Knock 82: Top-k Mask Prediction
==================================================
device: cuda
sentence: The movie was full of [MASK].
01	fun	0.107119
02	surprises	0.066345
03	drama	0.044684
04	stars	0.027217
05	laughs	0.025413
06	action	0.019517
07	excitement	0.019038
08	people	0.018290
09	tension	0.015031
10	music	0.014646
'''
