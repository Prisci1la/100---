'''
knock84.py: 平均による文ベクトル / 平均による文ベクトル

最終層のtoken埋め込みを平均して文ベクトルを作り、全ペアのコサイン類似度を求める。
/ 最終層のtoken埋め込みを平均して文ベクトルを作り、全ペアのコサイン類似度を求める。
'''
...
def get_mean_embeddings(tokenizer, model, sentences=SIMILARITY_SENTENCES, device=None):  # 取得平均句向量 / 平均文ベクトルを取得する
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") if device is None else device  # 自动选择设备 / デバイスを自動選択する
    ...
    token_embeddings = outputs.last_hidden_state  # 取出最后一层所有token向量 / 最終層の全tokenベクトルを取る
    content_mask = (encoding["attention_mask"] == 1) & (encoding["special_tokens_mask"] == 0)  # 排除PAD和特殊token / PADと特殊tokenを除く
    content_mask = content_mask.unsqueeze(-1).to(token_embeddings.dtype)  # 扩展维度以便广播 / broadcast用に次元を増やす
    summed = (token_embeddings * content_mask).sum(dim=1)  # 累加有效token向量 / 有効tokenベクトルを合計する
    lengths = content_mask.sum(dim=1).clamp(min=1.0)  # 计算有效token数量 / 有効token数を計算する
    return summed / lengths  # 返回平均向量 / 平均ベクトルを返す
...
def main():  # 定义主函数 / メイン関数を定義する
    parser = argparse.ArgumentParser(description="knock84: sentence vectors by mean pooling")  # 创建参数解析器 / 引数パーサーを作る
    ...
    embeddings = get_mean_embeddings(tokenizer, model, SIMILARITY_SENTENCES, device=device)  # 取得平均向量 / 平均ベクトルを取得する
    results = cosine_similarity_pairs(embeddings, SIMILARITY_SENTENCES)  # 计算全组合相似度 / 全組み合わせの類似度を計算する

    print("=" * 50)  # 输出分隔线 / 区切り線を出力する
    print("Knock 84: Mean Sentence Vectors")  # 输出标题 / タイトルを出力する
    print("=" * 50)  # 输出分隔线 / 区切り線を出力する
...
実行結果 / 运行结果:
==================================================
Knock 84: Mean Sentence Vectors
==================================================
device: cuda
0.953455	The movie was full of fun.	|	The movie was full of excitement.
0.837855	The movie was full of fun.	|	The movie was full of crap.
0.804551	The movie was full of fun.	|	The movie was full of rubbish.
0.825631	The movie was full of excitement.	|	The movie was full of crap.
0.784942	The movie was full of excitement.	|	The movie was full of rubbish.
0.918173	The movie was full of crap.	|	The movie was full of rubbish.
'''
