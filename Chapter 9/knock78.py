'''
knock78.py: 单词嵌入的学习 / 単語埋め込みの学習

不固定预训练词嵌入矩阵，而是把embedding层也作为训练对象。
/ 学習済み単語埋め込み行列を固定せず、embedding層も学習対象にする。
'''
...
def add_common_arguments(parser):  # 添加命令行参数 / コマンドライン引数を追加する
    ...
    parser.add_argument("--lr", type=float, default=1e-4, help="learning rate")  # 微调用较小学習率 / 微調整用に小さい学習率
    return parser  # 返回parser / parserを返す


def main():  # 定义主函数 / メイン関数を定義する
    parser = argparse.ArgumentParser(description="knock78: fine-tune word embeddings")  # 创建参数解析器 / 引数パーサーを作る
    ...
    model = BoWClassifier(embedding_matrix, freeze_embeddings=False).to(device)  # embedding也设为可学习 / embeddingも学習可能にする
    ...
    optimizer = torch.optim.Adam(trainable_parameters, lr=args.lr)  # 同时优化embedding和分类层 / embeddingと分類層を同時に最適化する

    print("=" * 50)  # 输出分隔线 / 区切り線を出力する
    print("Knock 78: Fine-tune Embeddings")  # 输出标题 / タイトルを出力する
    print("=" * 50)  # 输出分隔线 / 区切り線を出力する
    print(f"device: {device}")  # 输出设备 / デバイスを出力する
    print(f"embedding trainable: {model.embedding.weight.requires_grad}")  # 确认embedding可学习 / embeddingが学習可能か確認する
...
Command: python knock78.py
Exit code: 0, elapsed: 9.73s
终端输出节选:
==================================================
