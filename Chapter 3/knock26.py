"""
26. 強調マークアップの除去 / 移除强调标记
25 の処理時に、テンプレートの値から MediaWiki の強調マークアップ
（弱い強調、強調、強い強調のすべて）を除去してテキストに変換する。
在 25 的处理基础上，从模板的值中移除 MediaWiki 的强调标记
（弱强调、强调、强强调全部）并转换为纯文本。

MediaWiki の強調マークアップ / MediaWiki 的强调标记：
  ''テキスト''     → 弱い強調 / 弱强调（斜体）
  '''テキスト'''   → 強調 / 强调（粗体）
  '''''テキスト''''' → 強い強調 / 强强调（粗斜体）
"""
...
def remove_emphasis(value: str) -> str:
    ...
    移除强调标记。
    """
    # 長いものから先に置換する必要がある / 必须先替换最长的（5个→3个→2个）
    # 2〜5個のシングルクォートにマッチ / 匹配 2 到 5 个单引号
