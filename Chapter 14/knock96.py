...
from chapter14_utils import CH13_PROCESSED_DIR, TransformerMT, TranslationDataset, collate_translation, create_padding_mask, evaluate_bleu_for_beam, get_device, load_token_lines, load_vocab, train_epoch  # 共用工具 / 共通ツール
...
def main():  # 主函数 / メイン関数
    ...
    loader = DataLoader(TranslationDataset(src_lines, tgt_lines, src_vocab, tgt_vocab), batch_size=args.batch_size, shuffle=True, collate_fn=collate_translation)  # loader / loader
    ...
    writer = SummaryWriter(args.log_dir)  # 创建writer / writerを作る
    for epoch in range(1, args.epochs + 1):  # epoch循环 / epochループ
        ...
        train_bleu, _train_preds = evaluate_bleu_for_beam(model, src_lines, train_ref, src_vocab, tgt_vocab, beam_size=1, max_examples=args.max_train_bleu_examples, device=device)  # 训练BLEU / 訓練BLEU
        dev_bleu, _dev_preds = evaluate_bleu_for_beam(model, dev_src, dev_ref, src_vocab, tgt_vocab, beam_size=1, max_examples=args.max_dev_examples, device=device)  # 开发BLEU / 開発BLEU
